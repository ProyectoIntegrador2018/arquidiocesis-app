...
var Screen = (props)=>{
	...
	useEffect(()=>{
		if(isNew){
			API.getParticipantes(capacitacion).then(p=>{
				setData(p);
			}).catch(err=>{
			...
			API.getCapacitacionAsistencia(capacitacion, date).then(d=>{
				setData(d.miembros);
	...
	var showOverwrite = ()=>{
		Alert.alert('Asistencia ya existe', 'Ya se ha tomado asistencia en esta fecha de este capacitación, ¿Deseas reemplazarla por esta?', [
	...
	var saveAsistencia = (force=false)=>{
		setSending(true);
		if(isNew){
			API.registerCapacitacionAsistencia(capacitacion, date, assistance, force).then(done=>{
				setSending(false);
				onAssistance(done);
			...
			API.saveCapacitacionAsistencia(capacitacion, date, assistance).then(done=>{
				if(done.deleted){
