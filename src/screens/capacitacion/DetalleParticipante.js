...
export default (props)=>{
	...
	if(error){
		return <ErrorView message="Hubo un error cargando el participante" retry={getParticipante} />
	}
	...
	var deleteParticipante = ()=>{
		Alert.alert('¿Eliminar participante?', 'Esto eliminará sus datos de la capacitación.', [
	...
	return (
		<KeyboardAwareScrollView contentContainerStyle={{ paddingBottom: 50 }}>
			<View style={styles.container}>
				...
				<Input name="Apellido Paterno" value={participante.apellido_paterno} readonly/>
				<Input name="Apellido Materno" value={participante.apellido_materno} readonly/>
				<Input name="Nombre Corto" value={participante.nombre_corto} readonly/>
				<Input name="Sexo" value={participante.sexo} readonly />
				<Input name="Fecha de nacimiento" value={formatUnix(participante.fecha_nacimiento._seconds)} readonly />
				...
				<Input name="Grado escolaridad" value={participante.escolaridad} readonly />
				<Input name="Oficio" value={participante.oficio} readonly />

				<Text style={styles.section}>Domicilio</Text> 
				<Input name="Domicilio" value={participante.domicilio.domicilio} readonly />
				<Input name="Colonia" value={participante.domicilio.colonia} readonly />
				<Input name="Municipio" value={participante.domicilio.municipio} readonly />
				<Input name="Teléfono Casa" value={participante.domicilio.telefono_casa} readonly />
				<Input name="Teléfono Móvil" value={participante.domicilio.telefono_movil} readonly />
			</View>
			<Item text="Eliminar participante" onPress={deleteParticipante} loading={deleting} />
