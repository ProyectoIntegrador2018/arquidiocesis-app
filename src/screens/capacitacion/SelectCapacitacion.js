...
export default (props)=>{
	...
	var orderCapacitaciones = ()=>{
		...
		var active = data.filter(a=>(a.fin && a.fin._seconds && moment.unix(a.fin._seconds).endOf('day').unix()>today));
	...
	var renderCapacitacion = a=>{
		return <View>
			...
			<View style={{ flexDirection: 'row', alignItems: 'center' }}>
				<Text style={styles.date}>{formatUnix(a.inicio ? a.inicio._seconds : null)}</Text>
				<FontAwesome5 size={12} color={'gray'} name="chevron-right" style={{ marginTop: 2, marginHorizontal: 8 }} />
				<Text style={styles.date}>{formatUnix(a.fin ? a.fin._seconds : null)}</Text>
			</View>
		</View>
	}
					...
					<>
						{caps.active.length>0 ? (
							<View>
								...
								<List data={caps.active} onSelect={selectCapacitacion} scroll headers={false} renderItem={renderCapacitacion} scroll={false} />
							</View>
						) : null}
						{caps.old.length>0 ? (
							<View>
								<Text style={styles.header}>Capacitaciones pasadas</Text>
								<List data={caps.old} onSelect={selectCapacitacion} scroll headers={false} renderItem={renderCapacitacion} scroll={false}/>
							</View>
						) : null}
					</>
				) : (
