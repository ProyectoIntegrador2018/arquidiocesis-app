...
export default (props)=>{
	var parroquia_id = props.route.params.grupo.capilla ? props.route.params.grupo.capilla.parroquia.id : props.route.params.grupo.parroquia.id
	...
	var getCapillaIndex = ()=>{
		...
		return capillasList.findIndex(a=>a.id==props.route.params.grupo.capilla.id);
	}
	...
	var save = ()=>{
		...
		var { valid, prompt } = Util.validateForm(data, {
			nombre: { type: 'minLength', value: 3, prompt: 'Favor de introducir el nombre del grupo. Mínimo 3 caracteres.' },
			// coordinador: { type: 'empty', prompt: 'Favor de seleccionar el coordinador.' },
			parroquia: { type: 'empty', prompt: 'Favor de seleccionar la parroquia.' }
		})
		if(!valid) return alert(prompt);
		if(isEnabled && !capilla){
			return alert("Favor de seleccionar una capilla, o quitar la opción de pertenece a capilla.");
		}

		setLoading(true);
		API.editGrupo(props.route.params.grupo.id, data).then(done=>{
			setLoading(false);
				...
				capillasList ? (
					capillasList.length>0 ? (
						<Picker 
							...
							style={{ marginTop: 15 }}
							items={capillasList.map(a=>({ label: a.nombre, value: a.id }))} 
