...
export default (props)=>{
	...
	useEffect(()=>{
		API.getGrupoBajasTemporales(id).then(d=>{
			setData(d);
			setRefreshing(false);
			setError(false);
		}).catch(err=>{
	...
	var getMiembros = ()=>{
		setRefreshing(true);
		API.getGrupoBajasTemporales(id, true).then(d=>{
			setData(d);
			setError(false);
			setRefreshing(false);
		}).catch(err=>{
	...
	return <ScrollView style={{ flex: 1 }} refreshControl={
		...
		{error ? (
			<ErrorView message={'Hubo un error cargando las parroquias...'} refreshing={refreshing} retry={getMiembros} />
		) : data===false ? (
			<LoadingView />
		) : (
			<View>
				{data.length==0 ? (
					...
					<AlphabetList data={data} onSelect={viewMiembro} scroll sort={'nombre'} />
				)}
			</View>
		)}
	</ScrollView>
